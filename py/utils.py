...
class LogSettingRunner:
    def run():
        ...
        # ログの設定
        logzero.logfile(os.path.join(log_directory, log_file_name))  # ログファイルのパスを指定
...
def create_df_long(sheet: list) -> pd.DataFrame:
    ...
    tmp = tmp.melt(id_vars=["date", "weekday"], var_name="calendar", value_name="time")
...
def correct_bad_records(df: pd.DataFrame, bad_rec_index: list) -> pd.DataFrame:
    for idx in bad_rec_index:
        ...
        if t.count("#NUM!") == 0:
            ...
            i = t.index("#NUM!")
            complement = str(24.00 - sum([float(v) for v in t if v != "#NUM!"]))
            df.loc[idx] = [d, w, c, complement]
        else:
            c = t.count("#NUM!")
            complement = str((24.00 - sum([float(v) for v in t if v != "#NUM!"])) / c)
...
def output_graph(df:pd.DataFrame, window: int, save:bool=False):
    ...
    # 積み上げ棒グラフを描画
    ax = df.plot(kind="bar", stacked=True, figsize=(10, 7), color=color, edgecolor="#00081A", alpha=0.75)
    ...
    if save:
        ...
        if window == 7:
            ...
            logger.warning(f"bad window config is set.")
