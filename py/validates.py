...
def validate_data_types(df):
    ...
    for column, expected_type in expected_data_types.items():
        if df[column].dtype != expected_type:
            raise ValueError(
                f"Column '{column}' should be of type {expected_type}, but it is {df[column].dtype}"
            )

    print("Data types validation passed!")

def validates_calculate_moving_average(df_long: pd.DataFrame, df_tmp: pd.DataFrame, check_date: str, window:int):
    ...
    expected = df_tmp_20240101[df_tmp_20240101["calendar"] == "friends"]["time"].values[
        0
    ]
    assert actual == expected, AssertionError(
        f"""
